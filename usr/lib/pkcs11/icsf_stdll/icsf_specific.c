...
reset_token_data(CK_SLOT_ID slot_id, CK_CHAR_PTR pin, CK_ULONG pin_len)
{
	...
	if (compute_sha(pin, pin_len, nv_token_data->so_pin_sha, CKM_SHA_1)) {
			  ...
			  CK_CHAR_PTR label)
{
	...
	/* Check pin */
	rc = compute_sha(pin, pin_len, hash_sha, CKM_SHA_1);
...
token_specific_init_pin(SESSION *sess, CK_CHAR_PTR pPin, CK_ULONG ulPinLen)
{
	...
	rc = compute_sha(pPin, ulPinLen, hash_sha, CKM_SHA_1);
	if (rc != CKR_OK) {
		       ...
		       CK_CHAR_PTR pNewPin, CK_ULONG ulNewLen)
{
	...
	rc = compute_sha(pNewPin, ulNewLen, new_hash_sha, CKM_SHA_1);
	rc |= compute_sha(pOldPin, ulOldLen, old_hash_sha, CKM_SHA_1);
	if (rc != CKR_OK) {
		OCK_LOG_ERR(ERR_HASH_COMPUTATION);
		return rc;
	}
